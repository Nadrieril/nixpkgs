  ...
  options = {
    ...
    user.activationScripts = mkOption {
      ...
      description = ''
        ...
        profile is activated. Examples are updating
        <filename>$HOME</filename>, setting services, and so on. Since these
      ...
      apply = set: ''
          ...
          mkdir -p "$(dirname "$profileLink")"
          previousConfig="$(readlink -f "$profileLink")"
          ...
          # The readlink is there to ensure that when $userConfig = /system
